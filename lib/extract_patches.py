...
def get_data_training(DRIVE_train_imgs_original,
    ...
    ### for STARE ten folds cross validation
#     train_imgs_original, train_masks = np.delete(train_imgs_original,2,axis = 0), np.delete(train_masks,2,axis = 0)
...
def get_data_testing_overlap_wilcoxon(DRIVE_test_imgs_original, DRIVE_test_groudTruth, Imgs_to_test, patch_height, patch_width,
                             stride_height, stride_width):
    ### test
    test_imgs_original = load_hdf5(DRIVE_test_imgs_original)
    test_masks = load_hdf5(DRIVE_test_groudTruth)

    #     test_imgs_original = load_image()
    #     test_masks = load_gt()

    ### for STARE ten folds cross validation
    #     test_imgs_original, test_masks = test_imgs_original[-3:-1], test_masks[-3:-1]
    #     test_imgs_original, test_masks = test_imgs_original[0][np.newaxis, :], test_masks[0][np.newaxis, :]
    print("8888888888888888888", test_imgs_original.shape, test_masks.shape)
    ...
    test_imgs_shuffle = np.empty_like(test_imgs)
    test_masks_shuffle = np.empty_like(test_masks)
    first_temp = random.randint(0, 19)
    test_imgs_shuffle[0] = test_imgs[first_temp]
    test_masks_shuffle[0] = test_masks[first_temp]
    for i in range(20):
        temp = random.randint(0, 19)
        test_imgs_shuffle[i] = test_imgs[temp]
        test_masks_shuffle[i] = test_masks[temp]

    #     test_imgs = test_imgs[0:1, :, :, :]
    #     test_masks = test_masks[0:1, :, :, :]
    test_imgs_1 = paint_border_overlap(test_imgs_shuffle, patch_height, patch_width, stride_height, stride_width)

    # check masks are within 0-1
    assert (np.max(test_masks_shuffle) == 1 and np.min(test_masks_shuffle) == 0)

    print("\ntest images shape:")
    print(test_imgs_1.shape)
    print("\ntest mask shape:")
    print(test_masks_shuffle.shape)
    print("test images range (min-max): " + str(np.min(test_imgs_1)) + ' - ' + str(np.max(test_imgs_1)))
    print("test masks are within 0-1\n")

    # extract the TEST patches from the full images
    patches_imgs_test = extract_ordered_overlap(test_imgs_1, patch_height, patch_width, stride_height, stride_width)
    #     patches_imgs_test = test_imgs
    print("\ntest PATCHES images shape:")
    print(patches_imgs_test.shape)
    print("test PATCHES images range (min-max): " + str(np.min(patches_imgs_test)) + ' - ' + str(
    ...
    return patches_imgs_test, test_imgs_1.shape[2], test_imgs_1.shape[3], test_masks_shuffle
...
def extract_random(full_imgs, full_masks, patch_h, patch_w, N_patches, inside=True):
    ...
    print(N_patches, full_imgs.shape[1], patch_h, patch_w)
